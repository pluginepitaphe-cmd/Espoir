...
def create_demo_data():
    ...
    partner = User.query.filter_by(email="partenaire@example.com").first()
    if not partner:
        partner = User(
            email="partenaire@example.com",
            password_hash=generate_password_hash("part123"),
            first_name="Pierre",
            last_name="Durand",
            user_type="partner",
...
def register():
    try:
        ...
        return jsonify({"message": "Utilisateur enregistré avec succès", "user_id": new_user.id}), 201
...
def register():
    try:
        ...
        if not email or not password or not first_name or not last_name:
            return jsonify({'message': 'Veuillez remplir tous les champs obligatoires.'}), 400
